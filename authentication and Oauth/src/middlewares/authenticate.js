...
module.exports = async (req, res, next) => {
  ...
  {
    return res.status(400).send({
      message: "user_id is missing. Please enter user_id",
    });
  }
  else
  {
    const {role} = await User.findById(req.body?.user_id)
    if("seller".includes(role) || "admin".includes(role))
    {
      console.log()
    }
    else
    {
      return res.status(400).send({
        message: "user_id is not allowed, try with different",
      });
    }
  }
